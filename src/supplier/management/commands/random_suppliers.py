...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for n in range(*options['amount']):
            ...
            supplier.year_foundation = choice(range(1400, datetime.date.today().year + 1))
            ...
            for i in range(*options['amount_cars']):
                ...
                exists_cars = SupplierCarsPresence.objects.filter(supplier=supplier).values_list('car', flat=True)
        ...
        self.stdout.write(self.style.SUCCESS('Successfully created %s suppliers' % int(*options['amount'])))
