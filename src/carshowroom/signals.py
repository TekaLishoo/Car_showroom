...
from src.supplier.models import Car, SupplierCarsPresence, SupplierSales, Supplier
...
def run_after_saving(sender, instance, **kwargs):
    ...
    if not(instance.wish_cars.exists()):
        for car in Car.objects.filter(is_active=True):
            if instance.wishes["body_type"] == car.body_type and \
                    instance.wishes["drive_type"] == car.drive_type:
