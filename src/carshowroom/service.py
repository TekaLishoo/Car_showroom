from src.supplier.models import SupplierCarsPresence, Supplier
...
def showroom_find_best_price(car_id):
    ...
    """
    prices = dict()
    for offer in SupplierCarsPresence.objects.filter(
        is_active=True, car__id=car_id
    ).select_related("supplier"):
        offer_discount = 0
        for sale in offer.supplier.supplier_sales_supplier.filter(
            is_active=True,
...
def get_regular_customer_info(showroom):
    ...
    """
    number_sells_done = (
        CarShowroomPresence.objects.filter(is_active=True, showroom=showroom)
        .select_related("supplier")
        .values("supplier__id")
        .annotate(Sum("amount"))
    )
    number_sells_needed = Supplier.objects.filter(is_active=True).values(
        "id", "purchases_for_discount", "discount_regular_customer"
    )
    number_sells_dict = {}
    for supplier in number_sells_needed:
        if supplier["id"] in number_sells_done.values_list("supplier__id", flat=True):
            number_sells_dict[supplier["id"]] = [
                supplier["purchases_for_discount"]
                - number_sells_done.get(supplier__id=supplier["id"])["amount__sum"],
    ...
    for offer in prices:
        ...
        if offer[1][2] > cars_to_buy or offer[1][3] == 0:
            ...
            offers[offer[0]] = offer[1][2] * price_sale + (
                cars_to_buy - offer[1][2]
            ) * price_sale * (1 - offer[1][3] / 100)
